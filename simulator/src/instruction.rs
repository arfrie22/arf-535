...
impl Instruction {
    ...
    pub fn is_valid(&self) -> bool {
        match self {
            ...
            Self::IndirectJump { l, condition, rx, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::IndirectwithRegisterOffsetJump { l, condition, rx, ro, s } => *s < (1 << 4),
            Self::RelativeJump { l, condition, rx } => true,
            Self::ImmediateJump { l, condition, label } => *label < (1 << 16),
            Self::ImmediateRelativeJump { l, condition, offset } => *offset < (1 << 15) && *offset >= -(1 << 15),
            Self::IntegerLoadLow { rx, value } => *value < (1 << 16),
            Self::IntegerLoadHigh { rx, value } => *value < (1 << 16),
            Self::SwapIntegerRegisters { rx, ry } => true,
            Self::CopyIntegerRegister { rx, ry } => true,
            Self::LoadIntegerRegisterIndirect { rx, ry, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::LoadIntegerRegisterIndirectwithRegisterOffset { rx, ry, ro, s } => *s < (1 << 4),
            Self::LoadIntegerRegisterIndirectProgram { rx, ry, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::LoadIntegerRegisterIndirectwithRegisterOffsetProgram { rx, ry, ro, s } => *s < (1 << 4),
            Self::StoreIntegerRegisterIndirect { rx, ry, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::StoreIntegerRegisterIndirectwithRegisterOffsetIndirect { rx, ry, ro, s } => *s < (1 << 4),
            Self::StoreIntegerRegisterIndirectProgram { rx, ry, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::StoreIntegerRegisterIndirectwithRegisterOffsetProgram { rx, ry, ro, s } => *s < (1 << 4),
            Self::IntegerLoadData { rx, label } => *label < (1 << 16),
            Self::IntegerLoadProgram { rx, label } => *label < (1 << 16),
            Self::IntegerStoreData { rx, label } => *label < (1 << 16),
            Self::IntegerStoreProgram { rx, label } => *label < (1 << 16),
            Self::UnsignedZeroExtend { rx, ry, count } => *count < (1 << 5),
            Self::SignExtend { rx, ry, count } => *count < (1 << 5),
            Self::FloatingPointLoadLow { fx, value } => *value < (1 << 16),
            Self::FloatingPointLoadHigh { fx, value } => *value < (1 << 16),
            Self::SwapFloatingPointRegisters { fx, fy } => true,
            Self::CopyFloatingPointRegister { fx, fy } => true,
            Self::LoadFloatingPointRegisterIndirect { fx, ry, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::LoadFloatingPointRegisterIndirectwithRegisterOffset { fx, ry, ro, s } => *s < (1 << 4),
            Self::StoreFloatingPointRegisterIndirect { rx, fy, i, s } => *i < (1 << 5) && *s < (1 << 4),
            Self::StoreFloatingPointRegisterIndirectwithRegisterOffset { rx, fy, ro, s } => *s < (1 << 4),
            Self::FloatingPointLoadData { fx, label } => *label < (1 << 16),
            ...
            Self::LogicalShiftLeft { rx, ry, value } => *value < (1 << 5),
            Self::LogicalShiftRight { rx, ry, value } => *value < (1 << 5),
            Self::ArithmeticShiftLeft { rx, ry, value } => *value < (1 << 5),
            Self::ArithmeticShiftRight { rx, ry, value } => *value < (1 << 5),
            Self::RotateRight { rx, ry, value } => *value < (1 << 5),
