...
use crate::{parse_prog_label, parse_data_label, parse_number, parse_signed_number, AssemblerError, AssemblerParser, Rule};
        ...
        match t.as_rule() {
            Rule::EOI | Rule::label_arg => continue,
                ...
                Rule::instruction_STALL_06 => {
                    ...
                    instructions.push(Instruction::StallImmediate { value });
                }
                Rule::instruction_STALL_07 => {
                    let mut iter = t.into_inner();
                    let rx = iter.next().unwrap().as_str().parse()?;
                    instructions.push(Instruction::StallRegister { rx });
                }
