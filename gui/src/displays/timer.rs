...
impl TimerDisplay {
    ...
    fn cell_content_ui(&mut self, row_nr: u64, col_nr: usize, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            let timer = Timer::try_from(row_nr as usize).unwrap();
            if col_nr == 0 {
                ui.horizontal(|ui| {
                    ui.label(Into::<&'static str>::into(timer));
                });
            } else if col_nr == 1 {
                ui.horizontal(|ui| {
                    ui.label(format!("{:#08X}", self.state.borrow().timers[timer as usize].value))
                });
            } else if col_nr == 2 {
                ui.horizontal(|ui| {
                    ui.label(format!("{:#08X}", self.state.borrow().timers[timer as usize].previous_set))
                });
            } else {
                unreachable!()
            }
...
impl egui_table::TableDelegate for TimerDisplay {
