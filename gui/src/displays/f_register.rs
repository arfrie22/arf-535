...
use simulator::{enums::FPRegister, SimulatorState};
...
impl FRegisterDisplay {
    ...
    fn cell_content_ui(&mut self, row_nr: u64, col_nr: usize, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            let f_reg = FPRegister::try_from(row_nr as usize).unwrap();
            if col_nr == 0 {
                ui.horizontal(|ui| {
                    ui.label(Into::<&'static str>::into(f_reg));
                });
            } else if col_nr == 1 {
                ui.horizontal(|ui| {
                    ui.label(format!("{}", self.state.borrow().f_registers[f_reg as usize]))
                });
            } else {
                unreachable!()
            }
...
impl egui_table::TableDelegate for FRegisterDisplay {
...
impl PaneInner for FRegisterDisplay {
