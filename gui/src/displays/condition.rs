...
use simulator::{enums::{Register, Condition}, SimulatorState};
...
impl ConditionDisplay {
    ...
    fn cell_content_ui(&mut self, row_nr: u64, col_nr: usize, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            let cond = Condition::try_from(row_nr as usize).unwrap();
            if col_nr == 0 {
                ui.horizontal(|ui| {
                    ui.label(Into::<&'static str>::into(cond));
                });
            } else if col_nr == 1 {
                ui.horizontal(|ui| {
                    ui.label(format!("{}", cond.check(self.state.borrow().registers[Register::ST as usize])))
                });
            } else {
                unreachable!()
            }
...
impl egui_table::TableDelegate for ConditionDisplay {
...
impl PaneInner for ConditionDisplay {
